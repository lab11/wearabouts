...
for loc in data_dict:
    ...
    with open(loc + '.data') as f:
        for line in f:
            ...
            if loaded_data['rssi'] == -200:
                continue

            mac_addr = loaded_data['mac_addr']
            if mac_addr not in datas:
                datas[mac_addr] = {}
                datas[mac_addr]['uniqname'] = loaded_data['uniqname']
                datas[mac_addr]['full_name'] = loaded_data['full_name']
                datas[mac_addr]['rssi'] = []

            datas[mac_addr]['rssi'].append(loaded_data['rssi'])

lab_members = ['brghena', 'bradjc', 'wwhuang', 'bpkempke', 'sdebruin', 'ppannuto', 'samkuo', 'mclarkk', 'adkinsjd', 'tzachari', 'nealjack', 'immerman', 'rthaneda', 'ncnuech', 'nklugman']

for loc in data_dict:
    print(loc + '--')
    for mac_addr in data_dict[loc]:
        datas = data_dict[loc][mac_addr]
        print(str(datas['uniqname']) + ' - ' + str(len(datas['rssi'])))
    print('\n')
